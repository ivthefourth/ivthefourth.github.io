...
function drawSpectrum(){
	...
	eq.audio.analyser.getFloatFrequencyData(dataArray);
	...
	var barWidth; //= width / bufferLength;
	  ...
	  for(var i = 0; i < bufferLength; i++) {
	    barHeight = height * 1/(60)*((dataArray[i]+ 3*Math.log2(i + 1)) + 78);
	    x = width * Math.log2(i+1)/10;//if 1024 then 10 should be 9
	    barWidth = width * Math.log2(i+2)/10 - x +1;


	    //barHeight *=  (1+ Math.log2(i + 1)/10) // /*freq sensitivity -->*/ *(1024/(1300-i));
